...
public class NicknameSavedData extends PersistentState {
    ...
    public static NicknameSavedData fromNbt(NbtCompound nbt) {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            int legacyColor = playerNbt.contains("color", NbtElement.INT_TYPE) ? playerNbt.getInt("color") : -1;
            ...
            if (legacyColor >= 0 && !playerNicknames.current.isBlank()) {
                String previous = playerNicknames.current;
                playerNicknames.current = MiniMessageFormatter.applyLegacyColor(previous, legacyColor);
                int historyIndex = playerNicknames.history.indexOf(previous);
                if (historyIndex >= 0) {
                    playerNicknames.history.set(historyIndex, playerNicknames.current);
                }
            }
